...
public class BuyPillow {
    ...
    public BuyPillow(boolean previousRunPassed, Browser browser){
    ...
    String btnBuyNow = "//*[@class = \"btn full primary  btn-theme\"]";
    ...
    protected void run(){
        ...
        browser.click(btnBuyNow);

        browser.delay(20);

        browser.switchToIframe(iframe3DS);

        if(browser.verifyElementPresent(otpXpath)){
            System.out.println("Element found, test case " + className + " has passed");
            ConfirmPayment confirm = new ConfirmPayment(true, browser);
            confirm.run();
        } else {
